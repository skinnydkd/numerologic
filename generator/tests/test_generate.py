...
def test_pool_has_difficulty_spectrum():
    ...
    assert len(diffs) >= 2
    # dígits variables (5–7, grisos), brevi vàlid i tutti
    for p in pz:
        assert p["hasTutti"] is True
        assert p["brevi"]["count"] >= 1
        assert 5 <= len(p["digits"]) <= 7
        assert p["centralIndex"] < len(p["digits"])
        assert p["digits"][p["centralIndex"]] != 1  # el central mai és 1
    # hi ha reptes amb grisos (<7 dígits)
    assert any(len(p["digits"]) < 7 for p in pz)
